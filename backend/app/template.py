...
- 選択された原因について、以下の3つの異なる視点から具体的な事実を説明してください：
  1. 状況や環境に関する具体的な事実
  2. 行動や出来事の具体的な経過
  3. 結果として観察された具体的な事象
...
- explanationsは必ず配列（リスト）として出力してください
...
以下の失敗事例を踏まえつつ、その原因に関する説明を支持する根拠を5つ生成してください。
回答は後述のPydanticスキーマに従い、JSON形式で出力してください。
推測や創作は最小限に抑えてください。
出力は敬語を使わず、平易で直接的な表現にしてください。
...
以下の失敗事例を踏まえつつ、その原因に関する説明とそれを支持する根拠を、反証する可能性について、5つ生成してください。
回答は後述のPydanticスキーマに従い、JSON形式で出力してください。
推測や創作は最小限に抑えてください。
出力は敬語を使わず、平易で直接的な表現にしてください。
